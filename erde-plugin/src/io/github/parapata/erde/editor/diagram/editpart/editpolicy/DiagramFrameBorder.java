 ...
 */
public class DiagramFrameBorder extends FrameBorder {

    private static final int DELTA = 255 / 10;

    private Color color;

    public DiagramFrameBorder() {
        this(new Color(Display.getCurrent(), 0, 76, 153));
    }

    public DiagramFrameBorder(Color color) {
        super();
        this.color = color;
        ((TitleBarBorder) super.inner).setBackgroundColor(color);
    }

    public DiagramFrameBorder(String label, Color color) {
        super(label);
        this.color = color;
        ((TitleBarBorder) super.inner).setBackgroundColor(color);
    }

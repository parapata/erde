...
public class GenResourceEnumTemplate {
    ...
    protected final String TEXT_7 = NL + "    ;" + NL + "" + NL
            ...
            + NL + "" + NL + "    private String propKey;" + NL + "" + NL + "    private Resource(String propKey) {"
            + NL + "        this.propKey = propKey;" + NL + "    }" + NL + "" + NL + "    public String getValue() {"
            + NL + "        try {" + NL + "            return resource.getString(propKey);" + NL
            + "        } catch (MissingResourceException e) {" + NL + "            return propKey;" + NL + "        }"
            + NL + "    }" + NL + "" + NL + "    public String getValue(String defaultValue) {" + NL + "        try {"
            + NL + "            String result = resource.getString(propKey);" + NL
            + "            return StringUtils.isEmpty(result) ? defaultValue : result;" + NL
            + "        } catch (MissingResourceException e) {" + NL + "            return propKey;" + NL + "        }"
            + NL + "    }" + NL + "" + NL + "    public String createMessage(String... args) {" + NL
            + "        String message = resource.getString(propKey);" + NL
            + "        for (int i = 0; i < args.length; i++) {" + NL
            + "            message = message.replaceAll(String.format(\"\\\\{%d\\\\}\", i), args[i]);" + NL
            + "        }" + NL + "        return message;" + NL + "    }" + NL + "" + NL
            + "    public String getPropKey() {" + NL + "        return propKey;" + NL + "    }" + NL + "" + NL
            + "    public static Resource toResource(String propKey) {" + NL
            + "        return Arrays.stream(Resource.values())" + NL
            + "                .filter(key -> key.getPropKey().equals(propKey))" + NL + "                .findFirst()"
            + NL + "                .orElseThrow();" + NL + "    }" + NL + "}";
